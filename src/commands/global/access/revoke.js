...
module.exports.execute = async (client, interaction, { user }) => {
  await access.unset(user);

  const dUser = interaction.data.resolved.users[user], tag = `${dUser.username}#${dUser.discriminator}`;
  client.api.interactions(interaction.id, interaction.token).callback.post({ data: { type: 4, data: { content: `✅ User \`${tag}\` no longer has access to manage testing areas.` } } });
};